...
namespace Assignment4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //              select p;
            var partDes = invoices.OrderBy(p => p.PartDescription);
            ...
            //                orderby p.Price
            //                select p;
            var partPrice = invoices.OrderBy(p => p.Price);
            ...
            var partDesQuantity = invoices.OrderBy(q => q.Quantity).Select(s => new { First = s.PartDescription, Second = s.Quantity });
            ...
            var partValue = invoices.Select(s => new { s.PartDescription, InvoiceTotal = s.Quantity * s.Price}).OrderBy(o => o.InvoiceTotal).Select(v => new { First = v.PartDescription, Second = v.InvoiceTotal });
